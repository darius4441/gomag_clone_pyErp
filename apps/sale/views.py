...
class SaleOrderEditView(LoginRequiredMixin, FatherUpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.state.pk == 1:
            ...
            messages.warning(
                self.request,
                _("The current order %(order)s status does not allow updates.")
                % {"order": self.object.name},
            )
...
def sale_order_state(request, pk, state):
    ...
    if sale_order.state != 4:
        with transaction.atomic():
            ...
            if state.pk == 4:
                ...
                return redirect(
                    reverse_lazy(
                        "PyInvoice:detail", kwargs={"pk": invoice.pk, "type": 1}
                    )
                )
    else:
        messages.warning(
    ...
    return redirect(reverse_lazy("PySaleOrder:detail", kwargs={"pk": pk}))
...
def sale_order_to_invoice(request, sale_order):
    ...
    sale_order_detail = PySaleOrderDetail.objects.filter(sale_order_id=sale_order.pk)
