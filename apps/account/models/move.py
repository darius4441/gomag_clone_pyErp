...
# Thirdparty Library
from apps.core.models import PyCompany, PyFather
...
class PyAccountMove(PyFather):
    ...
    state = models.IntegerField(_("Status"), choices=ACCOUNT_MOVE_STATE, default=0)
    ...
    company_move = models.ForeignKey(
        PyCompany, on_delete=models.PROTECT, null=True, blank=True
    )
    reference = models.CharField(_("Reference"), max_length=80, null=True, blank=True)
    amount = models.DecimalField(_("Total"), max_digits=10, decimal_places=2, default=0)
    debit = models.DecimalField(_("debit"), max_digits=10, decimal_places=2, default=0)
    credit = models.DecimalField(
        _("credit"), max_digits=10, decimal_places=2, default=0
    )
...
class PyAccountMoveDetail(PyFather):
    ...
    account_move_id = models.ForeignKey(
        PyAccountMove, on_delete=models.PROTECT, verbose_name=_("account")
    )
    account_plan_id = models.ForeignKey(PyAccountPlan, on_delete=models.PROTECT)
    reference_company = models.ForeignKey(
        PyCompany, on_delete=models.PROTECT, verbose_name=_("company")
    )
