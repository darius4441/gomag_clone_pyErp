        ...
        with transaction.atomic():
            ...
            if form.is_valid() and formset.is_valid():
                for obj in formset:
                    ...
                    if (
                        "DELETE" in obj.cleaned_data.keys()
                        and obj.cleaned_data["DELETE"] is False
                    ):
                        t_debit += obj.cleaned_data["debit"]
                        t_credit += obj.cleaned_data["credit"]
                if t_debit != t_credit:
                    messages.error(self.request, _("Unbalanced accounting entry"))
...
class AccountMoveUpdateView(LoginRequiredMixin, FatherUpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.state == 0:
            ...
            messages.warning(
                self.request,
                _("The current move %(order)s status does not allow updates.")
...
def move_state(request, pk, state):
    ...
    invoice.state = state
    invoice.save()
    return redirect(reverse_lazy("PyAccountMove:detail", kwargs={"pk": pk}))
