...
# Thirdparty Library
from apps.core.models import PyProduct, PyTax
from apps.core.views.web_father import (
    FatherCreateView,
    FatherDetailView,
    FatherListView,
    FatherUpdateView,
...
class InvoiceListView(LoginRequiredMixin, FatherListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["add_url"] = reverse_lazy(
            "{}:add".format(object_name), kwargs={"type": _type}
        )
        context["type"] = _type
        if _type == 1:
            context["breadcrumbs"] = [{"url": False, "name": _("Customer Invoice")}]
        elif _type == 2:
            context["breadcrumbs"] = [{"url": False, "name": _("Provider Invoice")}]
        return context
    ...
    def get_queryset(self):
        ...
        if self.model._meta.object_name in self.EXLUDE_FROM_FILTER:
            ...
            queryset = self.model.objects.filter(
                company_id=self.request.user.active_company_id, active=True, type=_type
            )
        return queryset
...
class InvoiceDetailView(LoginRequiredMixin, FatherDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["back_url"] = reverse_lazy(
            "{}:list".format(object_name), kwargs={"type": _type}
        )
        ...
        context["breadcrumbs"] = [
            ...
            {"url": False, "name": self.object.name},
        ]
...
class InvoiceCreateView(LoginRequiredMixin, FatherCreateView):
    ...
    def get_success_url(self):
        _type = self.kwargs["type"]
        return reverse_lazy(
            "PyInvoice:detail", kwargs={"pk": self.object.pk, "type": _type}
        )
...
class InvoiceUpdateView(LoginRequiredMixin, FatherUpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.state.pk == 1:
            ...
            messages.warning(
                self.request,
                _("The current invoice %(obj)s status does not allow updates.")
...
class InvoiceDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["delete_message"] = (
            "<p>¿Está seguro de eliminar la orden de compras <strong>"
            + self.object.name
            + "</strong>?</p>"
        )
        context["cant_delete_message"] = (
            "<p>La orden de compras <strong>"
            + self.object.name
            + "</strong>, no puede ser eliminada.</p>"
        )
...
def invoice_state(request, pk, state, type):
    ...
    if invoice.state != 4:
        ...
        messages.warning(
            self.request,
            _("The current invoice %(invoice)s status does not allow updates.")
            % {"invoice": self.object.name},
        )
    return redirect(reverse_lazy("PyInvoice:detail", kwargs={"pk": pk, "type": type}))
