...
class PyFather(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "{}:detail".format(self._meta.object_name), kwargs={"pk": self.pk}
        )
    ...
    def delete(self, *args, **kwargs):
        try:
            ...
            message = _(
                "'The %(obj_name)s cannot be deleted, certain information on system depends on this.'"
            ) % {"obj_name": self._meta.verbose_name}
    ...
    def LoadData(cls, type, company_id):
        ...
        folder_apps = (
            format(settings.BASE_DIR) + "/apps/" + format(app_name) + "/" + type
        )
