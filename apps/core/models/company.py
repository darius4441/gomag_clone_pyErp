...
class PyCompany(PyFather):
    ...
    currency_id = models.ForeignKey(
        PyCurrency, null=True, blank=True, on_delete=models.PROTECT
    )
    ...
    content_wrapper_color = models.CharField(
        max_length=20, blank=True, default=giveme_content_wrapper_color
    )
    ...
    def create(cls, name, country, currency, company_id):
        ...
        if company_id:
            pycompany = cls(
                name=name, country=country, currency_id=currency, company_id=company_id
            )
        else:
