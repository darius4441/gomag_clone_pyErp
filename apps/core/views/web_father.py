...
class FatherListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["breadcrumbs"] = [{"url": False, "name": "{}".format(verbose_name)}]
        return context
    ...
    def get_queryset(self):
        if self.model._meta.object_name in self.EXLUDE_FROM_FILTER:
            ...
            queryset = self.model.objects.filter(
                company_id=self.request.user.active_company_id, active=True
            )
        return queryset
...
class FatherDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["update_url"] = reverse_lazy(
            "{}:update".format(object_name), kwargs={"pk": self.object.pk}
        )
        ...
        if forward:
            context["forward"] = reverse_lazy(
                "{}:detail".format(object_name), kwargs={"pk": forward.pk}
            )
        if backward:
            context["backward"] = reverse_lazy(
                "{}:detail".format(object_name), kwargs={"pk": backward.pk}
            )
        context["activate_form"] = ActivateForm(
            initial={"object_name": object_name, "object_pk": self.kwargs["pk"]}
        )
        return context
    ...
    def get_object(self, queryset=None):
        ...
        queryset = queryset.filter(
            pk=pk, active=True, company_id=self.request.user.active_company_id
        )
...
class FatherUpdateView(UpdateView):
    def get_context_data(self, **kwargs):
        ...
        context["back_url"] = reverse_lazy(
            "{}:detail".format(object_name), kwargs={"pk": self.object.pk}
        )
        ...
        if "pk" in self.kwargs:
            ...
            if forward:
                context["forward"] = reverse_lazy(
                    "{}:update".format(object_name), kwargs={"pk": forward.pk}
                )
            if backward:
                context["backward"] = reverse_lazy(
                    "{}:update".format(object_name), kwargs={"pk": backward.pk}
                )
        context["breadcrumbs"] = [
            {
                "url": reverse_lazy("{}:list".format(object_name)),
...
class FatherDeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["delete_message"] = _(
            "Are you sure to delete <strong>%(obj_name)s</strong>?"
