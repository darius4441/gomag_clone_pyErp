...
# Localfolder Library
from ..serializers.usercustom import UserCustomSerializer
from ..forms import (
    AvatarForm,
...
from ..models import PyCompany, PyMeta, PyParameter, PyPlugin, PyUser, PyWParameter
...
def cambio_clave(request):
    ...
    return render(request, "usercustom/password_change_form.html", context)
        ...
        if "uidb64" in self.kwargs and "token" in self.kwargs:
            ...
            if user is not None:
                ...
                messages.success(
                    self.request,
                    _(
                        "Instructions to recover your password in %(app_name)s were sent to the email account %(email)s"
                    )
                    % {"app_name": settings.APP_NAME, "email": email},
                )
                ...
                return HttpResponseRedirect(reverse_lazy("PyUser:password-recovery"))
