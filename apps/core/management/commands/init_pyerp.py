...
# Thirdparty Library
from apps.core.models import PyCountry, PyCurrency, PyParameter, PyPlugin, PyWParameter


class Command(BaseCommand):
    """Clase para inicialización de PyERP"""

    help = _("Command to initialize PyErp")

    def handle(self, *args, **options):
        # ================================================================== #
        self.stdout.write(
            self.style.MIGRATE_HEADING(_("*** Generating core migrations..."))
        )
        call_command("makemigrations", "core", interactive=False)

        # ================================================================== #
        self.stdout.write(
            self.style.MIGRATE_HEADING(_("*** Migrating the core database..."))
        )
        call_command("migrate", interactive=False)

        # ================================================================== #
        self.stdout.write(
            self.style.MIGRATE_HEADING(_("*** Loading PypErp country object..."))
        )
        ...
        # ================================================================== #
        self.stdout.write(
            self.style.MIGRATE_HEADING(_("*** Loading PypErp currency object..."))
        )
        ...
        PyParameter.objects.get_or_create(name="mail_catchall_alias", value="catchall")
        self.stdout.write("Installed 1 object(s)")

        # ================================================================== #
        self.stdout.write(
            self.style.MIGRATE_HEADING(_("*** Loading PypErp web parameter object..."))
        )
        PyWParameter.objects.get_or_create(name="register_user", value="True")
        self.stdout.write("Installed 1 object(s)")

        # ================================================================== #
        self.stdout.write(
            self.style.MIGRATE_HEADING(_("*** Loading PypErp plugins library..."))
        )
