...
class InitForm(forms.ModelForm):
    ...
    email = forms.EmailField(
        widget=forms.EmailInput(attrs={"placeholder": _("Admin email")})
    )
    password = forms.CharField(
        max_length=100,
        widget=forms.PasswordInput(attrs={"placeholder": _("Admin Password")}),
    )

    class Meta:
        model = PyCompany
        fields = ["name", "country", "email", "password"]
        labels = {
            "name": _("Company Name"),
            "country": _("Country"),
