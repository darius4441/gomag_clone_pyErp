...
def purchase_order_pdf(request, pk):
    ...
    products = PyPurchaseOrderDetail.objects.filter(purchase_order_id=pk).only(
        "product_id", "description", "quantity", "price", "discount", "amount_total"
    )
    ...
    table = Table(data_header, colWidths=[7 * cm, 3 * cm, 3 * cm, 3 * cm, 3.5 * cm])
    table.setStyle(
        TableStyle(
            [
    ...
    if products:
        ...
        for product in products:
            data_table.append(
                [
                    ...
                    product.discount,
                    locale.format(
                        "%.2f", product.amount_total, grouping=True, monetary=True
                    ),
                ]
            )
            high = high - 18
        ...
        table = Table(data_table, colWidths=[7 * cm, 3 * cm, 3 * cm, 3 * cm, 3.5 * cm])
        table.setStyle(
            TableStyle(
                [
                    # ('INNERGRID', (0, 0), (-1, -1), 0.25, colors.black),
    ...
    data_foot.append(
        [
            "",
            _("Net Amount or Affection") + ":",
            "$ "
            + locale.format(
                "%.2f", purchase_order.amount_untaxed, grouping=True, monetary=True
            ),
        ]
    )
    data_foot.append(
        [
            ...
            "$ "
            + locale.format(
                "%.2f", purchase_order.amount_exempt, grouping=True, monetary=True
            ),
        ]
    )
    data_foot.append(
        [
            ...
            "$ "
            + locale.format(
                "%.2f", purchase_order.amount_tax_iva, grouping=True, monetary=True
            ),
        ]
    )
    data_foot.append(
        [
            ...
            "$ "
            + locale.format(
                "%.2f", purchase_order.amount_tax_other, grouping=True, monetary=True
            ),
        ]
    )
    data_foot.append(
        [
            ...
            "$ "
            + locale.format(
                "%.2f", purchase_order.amount_tax_total, grouping=True, monetary=True
            ),
        ]
    )
    data_foot.append(
        [
            ...
            "$ "
            + locale.format(
                "%.2f", purchase_order.amount_total, grouping=True, monetary=True
            ),
        ]
    )
    ...
    high = high - (18 * 7)
    # Imprimir cuerpo la tabla
    table = Table(data_foot, colWidths=[11 * cm, 5 * cm, 3.5 * cm])
    table.setStyle(
        TableStyle(
            [
                # ('INNERGRID', (0, 0), (-1, -1), 0.25, colors.black),
